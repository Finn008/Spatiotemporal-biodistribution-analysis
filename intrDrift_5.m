function [Out]=intrDrift_5(FfilenameTotal,RfilenameTotal,In)
global W;
Out=struct;
...
TLfilename=['IntrDrift2_',FNameTable.Filename{'Original'},'_vs_',RNameTable.Filename{'Original'},'.ims'];
...
UmStartEnd=[Rfileinfo.UmStart{1}-10,Rfileinfo.UmEnd{1}+10];
Um=UmStartEnd(:,2)-UmStartEnd(:,1);
Pix=round(Um./Rfileinfo.Res{1}.*[0.5;0.5;1]);
...
J.Pix=Pix;
J.Res=Res;
J.UmStart=UmStartEnd(:,1); % Rfileinfo.UmStart{1};
J.UmEnd=UmStartEnd(:,2); % Rfileinfo.UmEnd{1};
J.Channels=2;
J.Timepoints=1;
J.Overwrite=1;
