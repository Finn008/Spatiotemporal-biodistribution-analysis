...
%     FilenameTotalCrude=regexprep(FilenameTotal,'.ims','_Outside.ims');
    ChannelTable=evalin('caller','ChannelTable;');
    ChannelTable{'ShowIntermediate','TargetFilename'}={'ExCDepp_M1_IhcCongoRed_Date201902121447_Outside.ims'};
    FilenameTotalCrude=ChannelTable{'ShowIntermediate','TargetFilename'};
    PixInter=ResLevels{'ShowIntermediate','Pix'}{1};
end

% Pix=size(Plaque).';
% if isequal(size(Outside),size(Plaque))==0
%     Outside=interpolate3D_3(Outside,Pix);
% end
Output=struct;
...
if ShowIntermediateSteps==1; ex2Imaris_2(interpolate3D_3(Plaque,PixInter),FilenameTotalCrude,Settings.Name{1}); end;

% if prod(Pix.*Res)>ThresholdLargeFile
%     [~,Plaque]=percentileFilter3D_3(Plaque,70,Res,[10;10;5],Outside,400,[500;500;Res(3)],1);
% else
[~,Plaque]=percentileFilter3D_3(Plaque,70,Res,ResLevels.Res{'percentileFilter3D'},Outside,400,[200;200;1],1);
% [~,Plaque]=percentileFilter3D_3(interpolate3D_3(Plaque,ResLevels.Pix{'percentileFilter3D'}),70,ResLevels.Res{'percentileFilter3D'},[],interpolate3D_3(Outside,ResLevels.Pix{'percentileFilter3D'}),400,[200;200;1],1);
% end
...
% if exist('ShowIntermediateSteps','Var') && ShowIntermediateSteps==1
    % % %     dataInspector3D(interpolate3D(Plaque,PixInter),Res2,'Plaque',1,FilenameTotalCrude,0);
...
PlaqueCore=(Plaque>=Threshold) & interpolate3D_3(Outside,Pix)==0; % previously factor 2
...
Wave1={'Res','Tiling','Overlap';max([1,min(Res)]),10^9,10}; Wave1=array2table(Wave1(2:end,:),'VariableNames',Wave1(1,:));
...
PlaqueMap=(Plaque>PlaqueLocalPrc & interpolate3D_3(Outside,Pix)==0 & DistanceFromCore<=10) | DistanceFromCore<=0;
...
ex2Imaris_2(PlaqueMap,ChannelTable.SourceFilename{'CongoRed'},'PlaqueMap');
imarisSaveHDFlock(ChannelTable.SourceFilename{'CongoRed'});
Application3=openImaris_4(ChannelTable.SourceFilename{'CongoRed'},[],1,1);
keyboard;
...
if strfind1(Settings.Properties.VariableNames.','SeparatePlaques',1) && strcmp(Settings.SeparatePlaques{1},'None')==0
    if strcmp(Settings.SeparatePlaques{1},'DistanceFromPlaqueBorder')
        ...
        if ShowIntermediateSteps==1
            ex2Imaris_2(interpolate3D_3(WatershedDistance,[],[],Pix),FilenameTotalCrude,'Plaque_WatershedDistance');
    ...
    if strcmp(Settings.SeparatePlaques{1},'Morphology')
        ...
        if ShowIntermediateSteps==1
            ex2Imaris_2(interpolate3D_3(PlaqueMapWatershed,[],[],Pix),FilenameTotalCrude,'Plaque_PlaqueMapWatershed');
            ex2Imaris_2(interpolate3D_3(LocalMaxima,PixInter),FilenameTotalCrude,'Plaque_LocalMaxima');
...
%             keyboard;
    ...
    if strcmp(Settings.SeparatePlaques{1},'Intensity&Morphology')
        ...
        Wave1={'Res','Tiling','Overlap';ResCalc,50^9,100}; Wave1=array2table(Wave1(2:end,:),'VariableNames',Wave1(1,:));
        PlaqueLocalPrc=tiledProcessing_2(PlaqueMap,[],Res,Wave1,'watershedSegmentation_2',CalcSettings);
        %         ex2Imaris_2(interpolate3D_3(Wave1,[],[],Pix),Application2,'Plaque_WatershedData');
        %         ex2Imaris_2(interpolate3D_3(LocalMaxima,PixInter),Application2,'Plaque_LocalMaxima');
        %         ex2Ima,ris_2(interpolate3D_3(PlaqueMapWatershed,[],[],Pix),Application2,'Plaque_PlaqueMapWatershed');
        
        if ShowIntermediateSteps==1
            ex2Imaris_2(interpolate3D_3(WatershedData,[],[],Pix),FilenameTotalCrude,'Plaque_WatershedData');
            ex2Imaris_2(interpolate3D_3(LocalMaxima,PixInter),FilenameTotalCrude,'Plaque_LocalMaxima');
        else
            clear WatershedDistance;
            clear LocalMaxima;
