% new to _9: RoiExclude
function [Output]=accumarray_10(Rois,Data,Function,OutputFormat,AccumMethod,CountInstances,RoiExclude)
...
if exist('RoiExclude') && isempty(RoiExclude)==0
    Exclude=false(size(Data));
    for m=1:size(RoiExclude,1)
        Exclude(Rois.Data{RoiExclude{m,1}}==RoiExclude{m,2})=1;
    end
    Data=Data(Exclude==0);
    for Row=1:size(Rois,1)
        Rois.Data{Row}=Rois.Data{Row}(Exclude==0);
    end
end
clear Exclude;
for Row=1:size(Rois,1)
...
if exist('AccumMethod')~=1 || isempty(AccumMethod)
