function [Out]=intrDrift_3(FfilenameTotal,RfilenameTotal,In)
global W;
Out=struct;
...
[Fileinfo]=getFileinfo_2(TLfilename);
...
if Report==1
    ...
    if isempty(Vobject)==0
        ...
        FitCoef=Fileinfo.Results{1}.MultiDimInfo.Var1{1}.U.FitCoefs(:,end);
...
if exist('FitCoef')==1 % overwrite FitCoef stored in Driftinfo
    if size(FitCoef,2)==1
...
if Out.Rmse1>10
    ...
    [Fileinfo,FileinfoInd]=extractFileinfo(TLfilename,Application,1);
...
% keyboard; % make automatic image
% J=struct;
% J.Application=Application;
% J.Path2file=[W.G.PathOut,'\IntrDrift\',FfilenameTotal,'_vs_',RfilenameTotal,'.tif'];
% imarisSnapshot(J)
...
% keyboard; % save Rotate also under Driftinfo
