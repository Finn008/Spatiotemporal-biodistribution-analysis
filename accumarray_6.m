function [Output]=accumarray_6(Rois,Data,Function,OutputFormat,AccumMethod)
...
Digits=0;
for m=1:size(Rois,1)
    [Rois.Unique{m},~,Rois.Data{m}]=unique(Rois.Data{m});
    [Rois.Unique{m},~,Rois.Data{m}]=unique(uint16(Rois.Data{m}));
    if max(Rois.Data{m})<=255
        Rois.Data{m}=uint8(Rois.Data{m});
    elseif max(Rois.Data{m})<=65535
        Rois.Data{m}=uint16(Rois.Data{m});
    else
        keyboard;
    end
