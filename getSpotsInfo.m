function [results]=getSpotsInfo(vSpots,properties)

Statistics = vSpots.GetStatistics;
...
for m=1:size(properties,1); % go through categories to extract
    % select elements of that property
%     wave1=strfind(Names,properties{m}); wave2 = ~cellfun(@isempty,wave1); wave1=find(wave2==1);
    wave1=strfind(Names,properties{m}); wave1 = ~cellfun(@isempty,wave1); 
    cP=all(wave1==1,:);
