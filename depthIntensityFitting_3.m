function [OrigData,Output]=depthIntensityFitting_3(Data,Res,Percentile,TargetValue,Masking,Outside,Path2file,EndVersion)
Pix=size(Data).';
OrigData=Data;
...
PercProfile=smooth(smooth(double(PercProfile)));
...
if max(Error)==0
    ...
    if strcmp(Type,'Max&Dev2Min')
        ...
        Wave1(Start:End,1)=0;
        Opts.Exclude=Wave1;
        [Fit,Gof]=fit(Zaxis,PercProfile,Ft,Opts);
    end
    
    FitCoefs=coeffvalues(Fit).';
    HalfDistance=log(0.5)/FitCoefs(2);
    StDev=Gof.rmse;
...
try; plot(X,(Fit(X)/Fit(X(Start))),'m'); end;

line([Start(1)*Res(3),Start(1)*Res(3)],[-1,1],'Color','k');
line([End*Res(3),End*Res(3)],[-1,1],'Color','k');
line([min(X),max(X)],[0,0],'Color','k');
set(gca,'ylim',[-1;1]);
Legend=legend('Raw','Dev1','Dev2','Dev3','Fit');
set(Legend,'color','none');
set(Legend,'location','northeastoutside');


text(X(round(size(X,1)/3)),0.9,[num2str(HalfDistance),'µm (',num2str(StDev),')'],'Color','m','FontSize',10);
% dim = [.2 .5 .3 .3];
