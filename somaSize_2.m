function somaSize_2(F,FilenameTotalOrig)
global W;
FilenameTotal=evalin('caller','FilenameTotal');
Res=evalin('caller','Res');
Pix=evalin('caller','Pix');
...
ex2Imaris_2(Out.DistInOut,FilenameTotal,'Calculation');

% select 'good' nuclei
Nucleus2BorderDistance=0.1; % should be 5um
...
J.SurfaceFilter=['"Volume" between 100 um^3 and 1500 um^3',['"Distance to Image Border XYZ" above ',num2str(Nucleus2BorderDistance),' um'],'"Sphericity" above 0.7 um^3'];
J.SeedsDiameter=8;
generateSurface3(J);
...
% % % ex2Imaris_2(Nuclei,FilenameTotal,'Test');
% % % Application=openImaris_2(FilenameTotal);
% % % Application.SetVisible(1);
...
ex2Imaris_2(NTperc,FilenameTotal,'NTperc');
Soma=NTperc>60;
...
ex2Imaris_2(Distance3D,FilenameTotal,'Calculation');
