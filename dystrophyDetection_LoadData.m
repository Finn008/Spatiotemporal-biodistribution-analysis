function [MouseInfo,SingleStacks]=dystrophyDetection_LoadData(TreatmentType,DataSelection)
...
for Row=1:size(SingleStacks,1)
    ...
    load(Path);
    Wave1=dir(Path);
    SingleStacks.Date(Row,1)={datestr(Wave1.datenum,'yyyy.mm.dd HH:MM')};
    SingleStacks.Res(Row,1)=Fileinfo.Res;
    ...
    try
        ...
        SingleStacks.Time2Treatment(Row,1)=Age-MouseInfo.StartTreatmentNum(Mouse);
        SingleStacks.TreatmentType(Row,1)=MouseInfo.TreatmentType(Mouse); % {Out.Tr};
    end
    ...
    try
        SingleStacks.PlaqueData(Row,1)={TotalResults.PlaqueData};
    catch
