function generateTimelineStacks_AllPlaquesPerTimepoint(Mouse,MouseInfo,PlaqueListSingle)
global W;
...
elseif Version==3 % Dystrophie2Radius, Plaque size as circonference in white
    ChannelInfo={'Dystrophies2Radius','D2DRatioA',{struct('ColorTable','jet','MinMax',[0;30])};...
...
for Roi=1:size(RoiInfo,1)
    ...
    for Time=1:size(Timepoints,1)
        ...
        for Ch=1:size(ChannelInfo,1)
            ...
            % Rotate
%             [DataOut,Mask,Out]=applyDrift2Data_4(In,SourceChannel)
        end
...
%         Figure=imshow(Image);
%         text(20,20,num2str(PlaqueListSingle3.Time2Treatment(Ind)),'Color',[1,1,1],'FontSize',20);
%         Wave1=im2frame(Figure);
%         Wave1=im2frame(gcf);
%         Wave1= getframe(gcf);
%         imshow(Wave1.cdata);
%         Wave1=getframe(Figure);
%         Wave1=frame2im(getframe(gcf,rec)); 
%         close Figure 1;
        try
