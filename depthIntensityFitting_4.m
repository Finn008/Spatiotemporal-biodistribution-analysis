function [OrigData,Output]=depthIntensityFitting_4(Data,Res,Percentile,TargetValue,Masking,Outside,Path2file,EndVersion)
Pix=size(Data).';
OrigData=Data;
...
% Threshold=[2;1.5;1.3;1.1;1.05;1.02];
% Threshold=[2;1.5];
Opts.Upper=[Inf,Inf,max(T.Y)];
Opts.Start=[max(T.Y),10,0];
Opts.Lower=[0,-Inf,0];
Figure=figure;
set(Figure,'WindowStyle','Docked')
for m=1:100
    T.Exclude(:)=1;
    T.Exclude(Start(m):End(m))=0;
