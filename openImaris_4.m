...
function [Application]=openImaris_4(PixMax,Res,SetVisible,MouseMove,UmMinMax,BitType)
global W;
mouseMoveController(1);
if ischar(PixMax)
   Path2file=PixMax;
else
    Path2file=W.Imaris.PathImarisSample;
end
Application=[];
...
if exist('BitType')==1 && strcmp(BitType,'uint16')==0
    if strcmp(BitType,'logical'); BitType='uint8'; end;
    Path2file=regexprep(Path2file,'.ims',['_',BitType,'.ims']);
...
% end
if exist('SetVisible')~=1 || isempty(SetVisible) || SetVisible==0
...
if exist('Res','Var')==1 && isempty(Res)==0
    UmMinMax=[[0;0;0],[Res.*PixMax(1:3)]];
end
