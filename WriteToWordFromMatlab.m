% provide table array, Text-column contains char code, TextColour, 
function WriteToWordFromMatlab(In)
v2struct(In);
global w;
w.wordVersion='german';
FullFilename=[Path2File,'\',Filename];
if strcmp(Overwrite,'replace')
    try; delete(FullFilename); end;
end
if exist('ActXWord')==0
%     WordFileName='TestDoc.doc';
    [ActXWord,WordHandle]=StartWord(FullFilename);
end

for m=1:size(WordTable,1)
    for n=1:size(WordTable.Text{m},2)
        addWordText(ActXWord,WordTable.Text{m}(1,n),Style,[0,2]);%two enters after text
    end
end
...
%     Style='Überschrift 1';
    Style='Überschrift 1'; %NOTE! if you are NOT using an English version of MSWord you get
...
global w;
    ...
    for r=1:nr_rows_p
        for c=1:nr_cols_p
            ...
            WordText(actx_word_p,data_cell_p{r,c},'Standard',[0,0]);
