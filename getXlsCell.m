function [Out]=getXlsCell(Sheet,Range,MyExcel,In)
...
Clock=datenum(now);
if exist('In')==0 || isempty(In)
    for Mcol=Range(2,1):Range(2,2)
        for Mrow=Range(1,1):Range(1,2)
            Wave1=[num2abc(Mcol),num2str(Mrow)];
            XlsCellinfo=MyExcel.Run('getCellInfo',Mrow,Mcol,Sheet);
            Cellinfo=cell2table(XlsCellinfo(2:end,:));
            Cellinfo.Properties.VariableNames=XlsCellinfo(1,:);
            Out{Mrow,Mcol}=Cellinfo;
